...
for j in range(photons) : #For each particle do the scattering loop
    ...
    while (norm<=R) :  #While we are inside the oject : do the scattering
        ...
        radius=int(R)
        n=np.zeros(radius+10) #Density
        i=0
        for j in range(0,radius+10):
            n[j]=1*(1-i/R)
            i+=1
        ...
        while ts<tau :
            ...
            if norm<=R :                        #Going forward by adding dire vector to the postion while inside the object
                ...
                dl=tau/(n[i]*sigma)/100 #Integration path
                N=int(norm)
        ...
        if norm<=R : 
            ts2=ts-(n[N+1]*sigma + n[N]*sigma)*dl/2   #Correction of the overshoot
            for i in range (0,3):               #
